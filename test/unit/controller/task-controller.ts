...
describe('TaskController', () => {
  ...
  beforeAll(async () => {
    ...
    const localUser = Object.assign(new User(), {
      id: 2,
      firstName: 'User',
      lastName: 'User',
  ...
  describe('GET /tasks', () => {
    ...
    it('returns 200 and validates the response shape', async () => {
      ...
      expect(res.body.records[0]).not.to.have.property('payload');
    });
    ...
    it('filters by multiple statuses and task type', async () => {
      await TaskService.dispatch(TEST_TASK_TYPE, { id: 1 });
      await TaskService.processNextEligible();
      await TaskService.dispatch(TEST_TASK_TYPE, { id: 2 });

      const res = await request(ctx.app)
        .get(`/tasks?status=${TaskStatus.PENDING},${TaskStatus.COMPLETED}&type=${TEST_TASK_TYPE}`)
        .set('Authorization', `Bearer ${ctx.adminToken}`);
    ...
    it('rejects invalid pagination', async () => {
      const res = await request(ctx.app)
        .get('/tasks?take=invalid')
        .set('Authorization', `Bearer ${ctx.adminToken}`);
      expect(res.status).to.equal(400);
    });

    it('returns 500 when tasks cannot be listed', async () => {
      sinon.stub(TaskService, 'getTasks').rejects(new Error('database unavailable'));
  ...
  describe('GET /tasks/:id', () => {
    ...
    it('returns 500 when the task cannot be loaded', async () => {
      sinon.stub(TaskService, 'getTask').rejects(new Error('database unavailable'));
  ...
  describe('POST /tasks/:id/retry', () => {
    ...
    it('returns 404 when retrying a missing task', async () => {
      const res = await request(ctx.app)
        .post('/tasks/999999/retry')
        .set('Authorization', `Bearer ${ctx.adminToken}`);
      expect(res.status).to.equal(404);
    });

    it('returns 500 when retrying the task fails unexpectedly', async () => {
      sinon.stub(TaskService, 'retry').rejects(new Error('database unavailable'));
  ...
  describe('GET /tasks/stats', () => {
    ...
    it('returns 500 when task statistics cannot be loaded', async () => {
      sinon.stub(TaskService, 'getStats').rejects(new Error('database unavailable'));
