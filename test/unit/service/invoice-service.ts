  ...
  describe('getTransactionInvoices', () => {
    it('should return invoices containing subtransaction rows from the transaction', async () => {
      await inUserContext((await UserFactory()).clone(2), async (debtor: User, creditor: User) => {
        // Create invoice with transfers - this creates transactions and then an invoice from them
        const invoice = await createInvoiceWithTransfers(debtor.id, creditor.id, 1);
        ...
        // Call getTransactionInvoices
        const invoices = await new InvoiceService().getTransactionInvoices(transaction.id);

        // Verify the invoice is returned
        expect(invoices).to.be.an('array');
        expect(invoices.length).to.equal(1);
        expect(invoices[0].id).to.equal(invoice.id);
        expect(invoices[0].to.id).to.equal(debtor.id);
      });
    });

    it('should return empty array when transaction has no invoices', async () => {
      await inUserContext((await UserFactory()).clone(2), async (debtor: User, creditor: User) => {
        // Create a transaction without creating an invoice
        const { transactions } = await createTransactions(debtor.id, creditor.id, 1);

        // Call getTransactionInvoices
        const invoices = await new InvoiceService().getTransactionInvoices(transactions[0].tId);
    ...
    it('should not return duplicate invoices when multiple rows from same transaction belong to same invoice', async () => {
      await inUserContext((await UserFactory()).clone(2), async (debtor: User, creditor: User) => {
        // Create invoice with multiple transactions (which will have multiple subtransaction rows)
        const invoice = await createInvoiceWithTransfers(debtor.id, creditor.id, 3);
        ...
        expect(totalRows).to.be.greaterThan(0);

        // Call getTransactionInvoices
        const invoices = await new InvoiceService().getTransactionInvoices(transaction.id);

        // Verify the invoice appears only once, even though multiple rows are in the invoice
        expect(invoices).to.be.an('array');
        expect(invoices.length).to.equal(1);
        expect(invoices[0].id).to.equal(invoice.id);
      });
    ...
    it('should return empty array when transaction does not exist', async () => {
      const nonExistentId = 999999;
      const invoices = await new InvoiceService().getTransactionInvoices(nonExistentId);
    ...
    it('should return an invoice whose transfer amount does not match the row sum', async () => {
      await inUserContext(
        ...
        async (debtor: User, creditor: User) => {
          ...
          expect(match.delta.getAmount()).to.equal(100);
    ...
    it('should return a deleted invoice when its credit transfer does not match the original transfer', async () => {
      await inUserContext(
        ...
        async (debtor: User, creditor: User) => {
          ...
          expect(match).to.not.be.undefined;
          expect(match.delta.getAmount()).to.equal(-300);
        },
      );
    });
    ...
    it('asInvoiceDriftResponse should embed a full BaseInvoiceResponse', async () => {
      await inUserContext(
        ...
        async (debtor: User, creditor: User) => {
          ...
          expect(response.delta.amount).to.equal(200);
