...
describe('TerminalPaymentService', () => {
  ...
  beforeAll(async () => {
    ...
    const { terminalPayments } = await new TerminalPaymentSeeder().seed([adminUser], [transactions.transactions[0]]);
  ...
  describe('#verifyTerminalPaymentRequest', () => {
    it('should use transaction validator', async () => {
      const transactionValidateStub = sinon.stub(TransactionService.prototype, 'verifyTransaction')
        .resolves({ valid: true });
      stubs.push(transactionValidateStub);

      const service = new TerminalPaymentService();
      const res = await service.verifyTerminalPaymentRequest({ transaction: ctx.validTransactionRequest });
      expect(res.valid).to.be.true;
      expect(transactionValidateStub).to.have.been.calledWith(ctx.validTransactionRequest);
    });
    ...
    it('should return a transaction context for a valid request', async () => {
      ...
      expect(res.context!.products.has(`${reqProduct.id}-${reqProduct.revision}`));
    });
    ...
    it('should reject an invalid transaction request', async () => {
      ...
      const service = new TerminalPaymentService();
      const res = await service.verifyTerminalPaymentRequest({ transaction: invalidTransactionReq });
  ...
  describe('#getTerminalPayment', () => {
    it('should return a CREATED terminal payment with the given id', async () => {
      const ctxTerminalPayment = ctx.terminalPayments.find((t) => t.getState() === TerminalPaymentState.CREATED);
      // Sanity check
      expect(ctxTerminalPayment, 'Precondition failed: could not find terminal payment with state "CREATED"').to.not.be.undefined;
      ...
      expect(dbTerminalPayment!.temporaryTransaction!.id).to.equal(ctxTerminalPayment!.temporaryTransaction!.id);
      expect(dbTerminalPayment!.stripePaymentIntent).to.not.be.null;
      expect(dbTerminalPayment!.stripePaymentIntent.id).to.equal(ctxTerminalPayment!.stripePaymentIntent.id);
      expect(dbTerminalPayment!.stripePaymentIntent.stripeId).to.equal(ctxTerminalPayment!.stripePaymentIntent.stripeId);
    });

    it('should return a PAID terminal payment with the given id', async () => {
      const ctxTerminalPayment = ctx.terminalPayments.find((t) => t.getState() === TerminalPaymentState.PAID);
      // Sanity check
      expect(ctxTerminalPayment, 'Precondition failed: could not find terminal payment with state "PAID"').to.not.be.undefined;
      ...
      expect(dbTerminalPayment!.transfer!.id).to.equal(ctxTerminalPayment!.transfer!.id);
      expect(dbTerminalPayment!.finalTransaction).to.not.be.null;
      expect(dbTerminalPayment!.finalTransaction!.id).to.equal(ctxTerminalPayment!.finalTransaction!.id);
      expect(dbTerminalPayment!.stripePaymentIntent).to.not.be.null;
      expect(dbTerminalPayment!.stripePaymentIntent.id).to.equal(ctxTerminalPayment!.stripePaymentIntent.id);
      expect(dbTerminalPayment!.stripePaymentIntent.stripeId).to.equal(ctxTerminalPayment!.stripePaymentIntent.stripeId);
    });
  ...
  describe('#createTerminalPayment', () => {
    it('should create a new TerminalPayment together with a TmpTransaction and Stripe payment intent', async () => {
      const req: CreateTerminalPaymentRequest = { transaction: ctx.validTransactionRequest };
      const service = new TerminalPaymentService();
      const { valid, context } = await service.verifyTerminalPaymentRequest(req);
      // Sanity checks
      expect(valid).to.be.true;
      expect(context).to.not.be.undefined;
      const terminalPayment = await service.createTerminalPayment(req, context!);
      ...
      const dbTerminalPayment = await ctx.connection.getRepository(TerminalPayment).findOne({ where: { id } });
      expect(dbTerminalPayment).to.not.be.null;

      // Correctly created Stripe PaymentIntent
      expect(paymentIntentsCreateStub).to.be.calledOnce;
      expect(terminalPayment.stripePaymentIntent.stripeId).to.equal(FAKE_PAYMENT_INTENT);

      // Cleanup
      await ctx.connection.getRepository(TerminalPayment).remove(terminalPayment);
    });
    it('should throw if the transaction request cannot be transformed into a transaction entity', async () => {
      ...
      await expect(promise).to.eventually.be.rejectedWith('SqliteError: NOT NULL constraint failed: tmp_transaction.createdById');
    });
  });
  ...
  describe('#startTerminalPayment', () => {
    it('should call the Stripe service with the matching payment intent id', async () => {
      ...
      const service = new TerminalPaymentService();
      const promise = service.startTerminalPayment(ctxTerminalPayment!.id, { stripeTerminalId: FAKE_READER_ID });
      await expect(promise).to.eventually.be.fulfilled;

      expect(readersProcessIntentStub).to.be.calledOnceWith(FAKE_READER_ID, { payment_intent: ctxTerminalPayment?.stripePaymentIntent.stripeId });
    });
    ...
    it('should throw if the terminal payment does not exist', async () => {
      ...
      const service = new TerminalPaymentService();
      const promise = service.startTerminalPayment(id, { stripeTerminalId: FAKE_READER_ID });
