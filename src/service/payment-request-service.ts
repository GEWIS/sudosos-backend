...
export default class PaymentRequestService extends WithManager {
   ...
   *
   * The caller is expected to have loaded the `paymentRequest` relation
   * (nested down to the `PaymentRequest` itself, for its eager `for`) on
   * `paymentIntent` already.
   */
  public async settlePaidStripeIntent(
    paymentIntent: StripePaymentIntent,
  ): Promise<PaymentRequest | null> {
    if (!paymentIntent.paymentRequest) return null;
    const request = paymentIntent.paymentRequest.paymentRequest;
