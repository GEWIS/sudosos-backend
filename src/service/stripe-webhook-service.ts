...
export default class StripeWebhookService extends WithManager {
  ...
  public async createNewPaymentIntentStatus(
    ...
    const paymentIntent = await this.manager.getRepository(StripePaymentIntent)
      .findOne({ where: { id: paymentIntentId }, relations: { deposit: true, paymentRequest: { paymentRequest: true }, terminalPayment: true } });
    if (!paymentIntent) {
    ...
    if (state === StripePaymentIntentState.SUCCEEDED && !!paymentIntent.paymentRequest) {
