...
/**
 * Persistent representation of a background task. The DB row is the source
 * of truth: the worker pool claims it atomically (see TaskService), runs
 * the registered handler, and writes the terminal state back here.
 *
 ...
 * @property {string} lockedBy - Identifier of the worker that currently owns the task.
 * @property {string} lockedUntil - Expiry time of the current worker lease (ISO string).
...
@Index('IDX_task_status_lockedUntil', ['status', 'lockedUntil'])
